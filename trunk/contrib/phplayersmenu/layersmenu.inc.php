<?php
// PHP Layers Menu 3.0.1 (C) 2001-2003 Marco Pratesi (marco at telug dot it)
...
* @return void
*/
function generateMenu($rows, $menu_name) {
	$this->_maxLevel[$menu_name] = 0;
	$this->_firstLevelCnt[$menu_name] = 0;
	...
	for($i=0;$i<count($rows);$i++) {
		$this->_tmpArray[$rows[$i]["id"]]["parent_id"] = $rows[$i]["parent_id"];
		$this->_tmpArray[$rows[$i]["id"]]["text"]      = $rows[$i]["text"];
		$this->_tmpArray[$rows[$i]["id"]]["link"]      = $rows[$i]["link"];
		$this->_tmpArray[$rows[$i]["id"]]["title"]     = $rows[$i]["title"];
		$this->_tmpArray[$rows[$i]["id"]]["icon"]      = $rows[$i]["icon"];
		$this->_tmpArray[$rows[$i]["id"]]["target"]    = $rows[$i]["target"];
		$this->_tmpArray[$rows[$i]["id"]]["expanded"]  = $rows[$i]["expanded"];
	}
